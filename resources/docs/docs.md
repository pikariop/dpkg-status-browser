...
The `home-routes` handler in the `dpkg-status-browser.routes.home` namespace
...
The `home-page` function will in turn call the `dpkg-status-browser.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `dpkg-status-browser.handler` namespace:
...
Request middleware functions are located under the `dpkg-status-browser.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `dpkg-status-browser.dev-middleware` namespace found in
